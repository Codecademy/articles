...
SET @period_start = CAST('2020-01-01' AS DATE)
SET @period_end = CAST('2020-01-31' AS DATE)

WITH start_subscribers (subscriber_id)
AS
(SELECT 
...
FROM
  subscribers 
WHERE
  subscribers.start_date < @period_start AND
  COALESCE(subscribers.end_date, @period_start) >= @period_start
),
...
WHERE
  subscribers.start_date < @period_start AND
  subscribers.end_date >= @period_start AND
  subscribers.end_date <= @period_end
)
